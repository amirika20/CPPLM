...
class EncodeInputs(nn.Module):
    ...
    def forward(
        ...
        intensity_embed = self.intensity_embed(intensity_ids.long()).unsqueeze(dim=1)
        intensity_embed = torch.zeros_like(intensity_embed, device="cuda")
        final_embedding = torch.cat((intensity_embed,sequence_embed), dim=1)
